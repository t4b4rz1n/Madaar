...
@receiver(pre_save, sender=OrganizationMembership, dispatch_uid="cache_org_salary_state_uid")
def cache_org_salary_state(sender, instance, **kwargs):
    """Cache the current salary values before save so we can detect changes."""
    if instance.pk:
        try:
            old = OrganizationMembership.all_objects.get(pk=instance.pk)
            instance.__original_salary_type = old.salary_type
            instance.__original_salary_amount = old.salary_amount
        except OrganizationMembership.DoesNotExist:
            instance.__original_salary_type = None
            instance.__original_salary_amount = None
    else:
        instance.__original_salary_type = None
        instance.__original_salary_amount = None


@receiver(post_save, sender=OrganizationMembership, dispatch_uid="sync_org_salary_to_projects_uid")
def sync_org_salary_to_projects(sender, instance, created, **kwargs):
    """When an org member's salary changes, propagate it to all project memberships
    that have NOT been individually overridden (salary_override=False).
    """
    new_type = instance.salary_type
    new_amount = instance.salary_amount

    old_type = getattr(instance, "__original_salary_type", None)
    old_amount = getattr(instance, "__original_salary_amount", None)

    # Skip if neither salary field changed
    if new_type == old_type and str(new_amount or "") == str(old_amount or ""):
        return
    ...
    try:
        ...
        updated_count = ProjectMember.objects.filter(
            user_id=instance.user_id,
            project__organization=instance.organization,
            salary_override=False,
            is_deleted=False,
        ).update(salary_type=new_type, salary_amount=new_amount)

        if updated_count:
            import logging
            logger = logging.getLogger(__name__)
            logger.info(
                "Propagated org salary to %d ProjectMember(s) for user %s in org %s",
                updated_count,
                instance.user_id,
        ...
        logging.getLogger(__name__).warning(
            "Failed to propagate org salary to projects for user %s: %s",
            instance.user_id,
            exc,
        )
