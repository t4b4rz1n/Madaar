...
from tasks.models import Board, TaskStatus
...
class AttendanceBaseTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        # Create users
        cls.owner = User.objects.create_user(username="owner", email="owner@example.com", password="password")
        cls.admin = User.objects.create_user(username="admin", email="admin@example.com", password="password")
        cls.lead = User.objects.create_user(username="lead", email="lead@example.com", password="password")
        cls.employee = User.objects.create_user(username="employee", email="employee@example.com", password="password")
        cls.other_employee = User.objects.create_user(username="other", email="other@example.com", password="password")
        ...
        # Memberships
        OrganizationMembership.objects.create(user=cls.owner, organization=cls.org, role="owner")
        OrganizationMembership.objects.create(user=cls.admin, organization=cls.org, role="admin")
        OrganizationMembership.objects.create(user=cls.lead, organization=cls.org, role="lead")
        OrganizationMembership.objects.create(user=cls.employee, organization=cls.org, role="employee")
        OrganizationMembership.objects.create(user=cls.other_employee, organization=cls.other_org, role="employee")
        ...
        # Projects and Tasks
        cls.project = Project.objects.create(name="Test Project", organization=cls.org, owner=cls.owner)
        board = Board.objects.create(project=cls.project, title="Test Board")
        status = TaskStatus.objects.create(board=board, name="To Do")
        cls.task = Task.objects.create(title="Test Task", project=cls.project, assignee=cls.employee, reporter=cls.lead, status=status)
...
class PermissionTests(AttendanceBaseTestCase):
    ...
    def test_timelog_owner_permission(self):
        ...
        if hasattr(request, '_user_org_roles_cache'): del request._user_org_roles_cache
...
class TimesheetTests(AttendanceBaseTestCase):
    ...
    def test_timesheet_viewset_project(self):
        self.client.force_authenticate(user=self.admin)
        TimeLog.objects.create(
            user=self.employee, task=self.task, project=self.project,
