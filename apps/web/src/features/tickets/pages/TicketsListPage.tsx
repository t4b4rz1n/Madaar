...
import { useTickets, useTicketTypes, useCreateTicketType, useUpdateTicketType, useDeleteTicketType } from "../hooks/useTickets";
...
export default function TicketsListPage() {
  ...
  const handleSearch = useCallback((q: string) => updateSearchParams("search", q), [updateSearchParams]);
  const handleSort = useCallback((k: string) => updateSearchParams("ordering", k), [updateSearchParams]);
  ...
  const handlePageChange = useCallback((p: number) => updateSearchParams("page", p.toString()), [updateSearchParams]);
  const handlePageSizeChange = useCallback((s: number) => updateSearchParams("page_size", s.toString()), [updateSearchParams]);
  const tickets = useMemo(() => ticketsResponse?.results || [], [ticketsResponse?.results]);

  // ── Categories state ───────────────────────────────────────────────────────
  const [catSearch, setCatSearch] = useState("");
  const [sortConfig, setSortConfig] = useState<{ key: "name"; dir: "asc" | "desc" }>({ key: "name", dir: "asc" });
  const debouncedCatSearch = useDebounce(catSearch, 500);

  const [catApiParams, setCatApiParams] = useState(() => new URLSearchParams({ ordering: "name" }));
  ...
  const { data: ticketTypesResponse, isLoading: catLoading, isError: catError } = useTicketTypes(catApiParams);
  ...
  const handleCatPageChange = useCallback((p: number) => {
    setCatApiParams((prev) => { const n = new URLSearchParams(prev); n.set("page", p.toString()); return n; });
  }, []);
  const handleCatPageSizeChange = useCallback((s: number) => {
    setCatApiParams((prev) => { const n = new URLSearchParams(prev); n.set("page_size", s.toString()); return n; });
  }, []);

  const ticketTypes = useMemo(() => ticketTypesResponse?.results || [], [ticketTypesResponse?.results]);

  const [modalState, setModalState] = useState<{ open: boolean; item: TicketTypeItem | null; name: string }>({
    open: false, item: null, name: "",
  });
  const [deleteModalState, setDeleteModalState] = useState<{ open: boolean; item: TicketTypeItem | null }>({
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2 text-base-content">
              {activeTab === "tickets" ? <Messages size={28} /> : <Category size={28} />}
          ...
          <div className="flex gap-1 p-1 bg-base-200 border border-base-content/10 rounded-xl w-fit">
            {tabs.map((tab) => (
              <button
                ...
                className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${activeTab === tab.id
              ...
              >
                ...
                {tab.count !== undefined && tab.count > 0 && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? "bg-primary-content/20 text-primary-content" : "bg-base-content/10 text-base-content/60"
                    }`}>
                    {tab.count}
                  </span>
                )}
        ...
        <AnimatePresence mode="wait">
          {activeTab === "tickets" ? (
            ...
            >
              <TicketsToolbar onSearch={handleSearch} onSortChange={handleSort} onFilterChange={handleFilter} />
              ...
              <div className="grow relative min-h-[200px]">
                ...
                <TicketsTable tickets={tickets} isLoading={isLoading} isError={isError} />
              </div>
              ...
              <div className="grow">
                {catLoading ? (
                  ...
                  <div className="bg-linear-to-br from-error/5 to-error/10 rounded-2xl border border-error/20 p-12 text-center">
                    ...
                    <p className="text-error/70">There was a problem loading categories</p>
                  </div>
                  ...
                  <div className="bg-linear-to-br from-base-200 to-base-300 rounded-2xl border border-base-content/10 p-12 text-center">
                    ...
                    <h3 className="text-lg font-bold text-base-content mb-2">No categories defined</h3>
                          ...
                          >
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <span className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0 border border-base-200">
                                  <Category size={18} className="text-primary" />
                                </span>
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={() => setDeleteModalState({ open: true, item })}
      ...
      {/* Create Ticket Modal */}
      <CreateTicketModal isOpen={modalOpen} onClose={() => setModalOpen(false)} />
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-base-content/70 text-sm">Ticket Category</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setModalState({ open: false, item: null, name: "" })}
